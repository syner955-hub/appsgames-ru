...
- НИКОГДА не упоминай источник, издание, автора или ссылку на оригинальный
  материал. Не пиши фразы вида "по данным X", "как сообщает Y", "источник:",
  "согласно материалу", "по версии редакции", "пишет издание". Материал должен
  читаться как наш собственный, без отсылок к внешним сайтам.
...
function buildWriterPrompt(item, imageDescription) {
  ...
  // Стиль картинок — фото-реалистичный product/lifestyle, а НЕ abstract 3D,
  // чтобы лента на главной не выглядела "AI-шной". Один из 4 вариантов выбирает
  // GPT-5 сам под сюжет. Общие ограничения (без текста, без логотипов,
  // без узнаваемых лиц) прописаны жёстко, чтобы обложки можно было публиковать.
  const styleHint =
    'Стиль картинки — реалистичная фотография (photorealistic), один из вариантов:' +
    ' (a) product photography смартфона на чистом столе,' +
    ' (b) close-up lifestyle смартфона в руке,' +
    ' (c) overhead flat-lay рабочего стола со смартфоном и 1-2 предметами,' +
    ' (d) macro-детали смартфона.' +
    ' 35mm или 50mm look, soft natural light, shallow depth of field, editorial tech magazine vibe.' +
    ' Без видимого текста на экране и в кадре, без узнаваемых логотипов брендов, без букв и watermark,' +
    ' без узнаваемых лиц людей (максимум — руки). Цветовую палитру подбирай под настроение новости.';

  const imagePromptRule = imageDescription
    ? `"imagePrompt": "Английский prompt для FLUX 2 Pro, 30-50 слов. ${styleHint} Тему и 2-3 ключевых объекта возьми из описания оригинальной картинки выше, чтобы обложка была семантически близкой, но это должна быть НАША оригинальная фотография — не копия."`
    : `"imagePrompt": "Английский prompt для FLUX 2 Pro, 25-45 слов. ${styleHint} Опиши конкретную сцену с 2-3 объектами в кадре и палитру под тему новости."`;
...
ТРЕБОВАНИЯ К JSON:
{
  ...
  "body": "Основной текст в markdown (1800-3000 символов). Можно использовать ## подзаголовки, **жирный**, списки, пронумерованные списки. Не вставляй заголовок h1 внутрь body — он уже в frontmatter. НЕ добавляй блок 'Источник:', 'По данным:' или ссылки на сторонние издания — текст должен читаться как наш собственный материал.",
